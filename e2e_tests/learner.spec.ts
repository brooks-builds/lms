...
test('navigation', async ({ page }) => {
  ...
  await page.locator('#navbarSupportedContent').getByRole('link', { name: 'Courses' }).click();
  await page.locator('#navbarSupportedContent').getByRole('link', { name: 'Home' }).click();
  await page.getByRole('heading', { name: 'Brooks Builds Learning' }).click();
});
...
test('Visitors can preview a course', async ({ page }) => {
  await page.goto('http://localhost:8082/');
  await page.locator('#navbarSupportedContent').getByRole('link', { name: 'Courses' }).click();
  await page.getByRole('heading', { name: 'Course Library' }).click();
  await page.getByRole('heading', { name: 'Featured Courses' }).click();
  await page.getByTestId('courses').getByRole('link', { name: / }).click();
